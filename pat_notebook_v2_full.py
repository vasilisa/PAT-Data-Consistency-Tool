#v20260502: Check 8 — breakdown Premium_% now shows % of total premium (mapped + unmapped), not % of unmapped only.

#v20260501: Check 8 — unmapped Key_Modelling rows now broken down by tbl_Key_Mapping identifier columns.

#v20260423b: tbl_Trend uniqueness uses all columns except Trend_Value
...
def _ensure_dd_aggregated(project, loaded_ref_tables):
    ...
    Ensure tbl_DetailedData_Agg exists, its recipe settings are current, and
    ...
    Every call:
      ...
      - Creates the output dataset and Group recipe if they do not exist
        ...
        (first run only — requires the flow to be set up in Dataiku)
      ...
      - If the dataset already exists: triggers a rebuild on the existing

        recipe so the checks always run against current data

      - If the dataset does not yet exist: triggers the first-time build
    ...
    # ── Rebuild existing dataset after settings update ────────────────────────

    if agg_exists:

        print("  Rebuilding tbl_DetailedData_Agg…", end=" ", flush=True)
        ...
        if state != "DONE":
            ...
            raise RuntimeError(
                ...
                f"Rebuild job ended with state '{state}'. "
...
def check_key_modelling_unmapped(dd_df, mapping_df=None):
    ...
    When mapping_df (tbl_Key_Mapping) is provided, the non-Key_* columns in

    that table are treated as unit identifiers. For each unmapped group the

    premium is broken down by those identifier columns so the user can see

    exactly which segments have no Key_Modelling assignment.
    ...
    # Non-Key_* columns from tbl_Key_Mapping that also exist in tbl_DetailedData

    # are the unit identifiers — the segmentation dimensions that should determine

    # which Key_Modelling a row belongs to.
    ...
    for col in sorted(c for c in dd_df.columns if c.startswith("Key_Modelling")):
        ...
        # Premium breakdown by identifier columns for unmapped rows
    ...
    for tbl in all_tables:
        ...
        if uniq.get("status") == "FAIL":
            ...
            if ref_df is not None:
                ...
                # tbl_Trend: all uniqueness cols are dimensions — show them all

                trend_cols = tuple(dups.columns) if tbl == "tbl_Trend" else ()

                dups = _trim_to_display(

                    dups, ref_df, dd_df,

                    extra_cols=year_cols + dev_cols + trend_cols,
...
def display_key_modelling(km_results):
    ...
    # Summary table — one row per Key_Modelling* column
    ...
    # Breakdown by identifier columns — shown for each WARNING column

    for r in km_results.values():

        bd = r.get("breakdown")

        if r["status"] != "WARNING" or bd is None or bd.empty:

            continue

        id_cols = r.get("id_cols", [])

        n_shown = min(20, len(bd))

        parts.append(

            f"<div class='dct-note' style='margin-top:8px'>"

            f"<b>{r['column']} — unmapped premium by segment</b> "

            f"(top {n_shown} of {len(bd)} combination(s), "

            f"identifier columns from tbl_Key_Mapping)</div>"

        )

        # Add Premium_% column — as % of total (mapped + unmapped) premium

        bd = bd.copy()

        bd["Premium_%"] = (bd["Premium"] / r["total_premium"] * 100).round(1)

        parts.append(_df_to_html(bd, max_rows=20))
...
def run_and_display_Data_Consistency():
    ...
    km_results     = check_key_modelling_unmapped(dd_df, mapping_df)
